...
#define COLOR_LIGHT_GRAY (Clay_Color){213, 213, 213, 255}
#define COLOR_RED (Clay_Color){255, 0, 0, 255}
#define COLOR_GREEN (Clay_Color){0, 255, 0, 255}

static void renderSidebar(State *state);
static void clickToggleBoolean(Clay_ElementId id, Clay_PointerData pointerData,
                               void *userData);
...
Clay_RenderCommandArray createUi(State *state, int fps, float deltaTime) {
  ...
  CLAY(CLAY_IDI("Root", 0),
    ...
    CLAY(CLAY_ID("MainContent"),
      ...
      Clay_String fpsClayString = {.chars = fpsText,
      ...
      CLAY_AUTO_ID({
        ...
        CLAY_TEXT(fpsClayString, {.fontId = FONT_ID_NORMAL_16,
                                  .fontSize = 26,
  ...
  return Clay_EndLayout(
      deltaTime); // deltaTime drives Clay's transition/animation API
}
...
static void renderSidebar(State *state) {
  CLAY(CLAY_IDI("Sidebar", 1),
    ...
    CLAY(CLAY_ID("CloseBar"),
         {
             ...
             .backgroundColor = Clay_Hovered() ? COLOR_LIGHT_GRAY : COLOR_WHITE,
             .cornerRadius = CLAY_CORNER_RADIUS(10),
         }) {
    ...
    if (state->sidebarIsVisible) {
      CLAY_TEXT(
          CLAY_STRING("Fractal visualizer"),
          {.fontId = FONT_ID_TITLE, .fontSize = 36, .textColor = COLOR_TEXT});
      CLAY(CLAY_ID("SidebarSpacer"),
           {.layout = {.sizing = {.height = CLAY_SIZING_FIXED(12)}}}) {}
...
static void VSpacer(int minWidth) {
  CLAY_AUTO_ID({.layout = {.sizing = {.height = CLAY_SIZING_FIXED(1),
                                      .width = CLAY_SIZING_FIXED(minWidth)}}}) {
  }
  CLAY_AUTO_ID({.layout = {.sizing = {.height = CLAY_SIZING_FIXED(1),
                                      .width = CLAY_SIZING_GROW(0)}}}) {}
}
